<template>
  <div>
    ...
    <el-dialog
      title="多级联动编辑"
      class="multiLevel-config-wrapper"
    ...
    >
      <div class="placeholder-wrapper">
        <div
          ...
          v-for="(item, i) in multilevelData.placeholder"
          :key="item.hash"
        >
          <div class="placeholder-wrapper-list">
            <div
              class="placeholder-disable-edit multiLevel-input"
            ...
            <el-input
              ...
              v-model="item.text"
              v-else
              class="multiLevel-input"
            />
          </div>
          <i-ep-ArrowRight
            v-if="multilevelData.placeholder.length - 1 > i"
          ...
          <template v-if="key == 0">
            <draggable :list="multilevelData?.children" itemKey="hash">
              <template #item="{ element, index }">
                <div
                  :class="`option-wrapper-item ${element.hash == multilevelVal[key]?.hash ? 'input-active' : ''}`"
                  :key="element.hash"
                  @click="setMultilevelVal(element, key)"
                >
                  <el-input v-model="element.text" class="multiLevel-input">
                    <template #suffix>
                      <i-ep-RemoveFilled
                        v-if="element.hash == multilevelVal[key]?.hash"
                        class="remove-icon"
      ...
      <div class="add-node-wrapper">
        <template v-for="(item, key) in multilevelVal" :key="key">
          <div
            v-if="key == 0 || multilevelVal[key - 1]"
            @click="addMultilevelNode(key)"
            class="add-node-item"
          >
            <i-ep-Plus />
            添加选项
